...
public partial class DataGrid
{
    ...
    public static readonly BindableProperty PageCountProperty =
        BindableProperty.Create(nameof(PageCount), typeof(int), typeof(DataGrid), 1,
            propertyChanged: (b, o, n) =>
            {
                if (o != n && b is DataGrid self && n is int pageCount && pageCount > 1)
                {
                   //if (pageCount>1) 
    ...
    private View GetHeaderViewForColumn(DataGridColumn column, int index)
    {
        ...
        if (IsSortable && column.SortingEnabled && column.IsSortable(this))
        {
            ...
            column.SortingIconContainer.WidthRequest = HeaderHeight * 0.1;
            //column.Filter.WidthRequest = HeaderHeight * 0.5;
            //column.HeaderLabel.WidthRequest = HeaderHeight * 0.5;
            ...
            var grid = new Grid
            {
                ...
                Children = { column.HeaderLabel, column.SortingIconContainer,column.Filter },
                GestureRecognizers =
                {
            ...
            Grid.SetColumn(column.Filter, 2);
            return grid;
        }
